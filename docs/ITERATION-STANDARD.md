...
| 12 | Referral | programme → code → invitation → attribution → qualification → entitlement |
...
| 11 | Group coordination | **PASS** | Certified — run [33075208430](https://github.com/olu3242/Las-Afrique/actions/runs/33075208430) on `04da68b`. Membership never widens a tenant policy: the group owns its own tables, and a schema test asserts no owner-scoped policy consults group membership, so a group's own owner still gets nothing for a member's trip. Readiness aggregates over sharing members only — a non-sharer is excluded from the arithmetic rather than counted as un-ready, and a test swings their private state from best to worst and requires the whole output to be identical. Coordination only: a schema test refuses any column implying custody of funds. Took eleven hosted runs — one real engine defect (a stale consent re-read clearing the state it should publish, fixed atomically in `7d5bff7`) and one test synchronisation defect (a reload racing the server action, fixed in `04da68b`). The database was right the whole time and the journey read the page too early |
| 12 | Referral | **PASS** | Certified — run [33402098954](https://github.com/olu3242/Las-Afrique/actions/runs/33402098954) on `294a578`, which is the run that carries the approved invitation limit; `b3bf9bd` certified the engine before that limit was corrected. Referral is separated from reward, so attribution, qualification and entitlement certify without deciding what a reward is worth; a schema test on both tiers refuses any column implying custody, PRD §8. Attribution is provenance a user causes but cannot dictate: `referrals` is readable by both parties and writable by neither — its insert, update and delete policies are explicitly `false` — and the two writes go through definer functions that resolve the referrer from a code the caller cannot read and evaluate the qualification predicate against the caller's own rows. The referrer sees a status and an address they typed themselves; a link shared in a chat names nobody. The approved decisions live in `docs/ITERATION-12-DECISIONS.md`; the invitation limit is 10 attempts per referrer per rolling hour with refused attempts counted (0015), which a row count could not express — a refused insert leaves no row, so probing was free. Seven hosted runs to green, then one more for 0015. Three found real defects the local tier cannot see: surplus `authenticated` grants a hosted project's default privileges add (0013), a 500 on every user's first `/referrals` render from calling a server action during render, and the plaintext invitation token stored as provenance beside a column deliberately hashed (0014). Two were assertions I wrote against a UI I had not rendered — a code read out of concatenated text, and a 404 matched on wording rather than status. One was a latent `click(); reload();` race left on Iteration 11's opt-out path, which my added latency in `createTrip` exposed |
| 13 | Native mobile apps | **OUT_OF_SEQUENCE** | Not an engine in this codebase. Moved to a platform roadmap; the web product should establish the domain contracts a future client consumes |
| 14 | Post-arrival AI concierge | **BLOCKED** | Specified in the PRD, blocked on the same provider credential as Iteration 6. Blocked, not unscoped — the capability is named, the execution is not available |
| 15 | — | **UNSCOPED** | No capability named. Left empty rather than filled |
| 16 | — | **UNSCOPED** | No capability named. Left empty rather than filled |
