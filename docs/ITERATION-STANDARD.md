...
| 12 | Referral | **SCOPE_REQUIRED** | Roadmap names "Referral and rewards"; no engine was specified. Stopped at scope discovery rather than inventing product behaviour. A proposal is in `docs/ITERATION-12-SCOPE.md`, separating referral from reward so attribution can be built without deciding that the product holds money — which PRD §8 forbids. Awaiting approval |
