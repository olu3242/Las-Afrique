...
| 12 | Referral | **HOSTED_PENDING** | Scope approved; engine built against migration `0012_referrals.sql`. Referral is separated from reward, so attribution, qualification and entitlement are complete without deciding what a reward is worth — PRD §8 is enforced as a schema test rather than a convention. Attribution is provenance a user causes but cannot dictate: `referrals` is readable by both parties and writable by neither, and both writes go through security-definer functions that establish the referrer and evaluate the predicate against the caller's real rows. Certification pending the hosted run |
